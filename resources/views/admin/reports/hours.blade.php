            ...
            <table class="w-full text-left">
                <thead class="bg-slate-50/50 text-slate-400 text-[10px] uppercase tracking-[0.2em] font-bold">
                    <tr>
                        ...
                        <th class="px-8 py-5">Meta Fase</th>
                        <th class="px-8 py-5">Esta Semana</th>
                        <th class="px-8 py-5">Recuperadas</th>
                        <th class="px-8 py-5 text-right">Progreso Fase</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @forelse($apprentices as $index => $apprentice)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-6">
                                <span class="text-sm font-bold text-slate-500 font-outfit">{{ $apprentice->formatMinutesToHours($apprentice->expected_minutes) }}</span>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6 text-right">
                                @php
                                    $progreso = $apprentice->compliance_percentage;
                                @endphp
                                <div class="flex flex-col items-end gap-1.5">
                                    <span class="text-[10px] font-black {{ $progreso >= 100 ? 'text-emerald-500' : ($progreso >= 50 ? 'text-blue-500' : 'text-slate-400') }} uppercase tracking-widest">{{ $progreso }}% Fase</span>
                                    <div class="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                        <div class="h-full {{ $progreso >= 100 ? 'bg-emerald-500' : ($progreso >= 50 ? 'bg-blue-500' : 'bg-sena') }} transition-all duration-1000" style="width: {{ min(100, $progreso) }}%"></div>
                                    </div>
                                </div>
                            </td>
                        </tr>
